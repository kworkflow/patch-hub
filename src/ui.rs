...
pub fn draw_ui(f: &mut Frame, app: &App) {
    ...
    match app.current_screen {
        CurrentScreen::MailingListSelection => {
            mail_list::render_mailing_list_selection(f, app, chunks[1])
        }
        CurrentScreen::BookmarkedPatchsets => {
            bookmarked::render_bookmarked_patchsets(f, &app.bookmarked_patchsets_state, chunks[1])
        }
        CurrentScreen::LatestPatchsets => latest::render_latest_patchsets_list(f, app, chunks[1]),
    ...
    navigation::render_navi_bar(f, app, chunks[2]);
}
