...
impl DetailsActions {
    ...
    fn apply_patches(&self, kernel_tree: &KernelTree, config: &Config) -> Result<(), String> {
    ...
    ///
    ///
    /// Returns a `String` containing either the success or error message.
    /// # TODO:
    /// - Add unit tests
    pub fn apply_patchset(&self, config: &Config) -> Result<String, String> {
        ...
        let git_am_result = self.apply_patches(kernel_tree, config);
