...
fn bat_patch_renderer(patch: &str) -> color_eyre::Result<String> {
    ...
    bat.stdin.as_mut().ok_or_else(|| eyre!("Failed to get stdin handle"))?.write_all(patch.as_bytes())?;
...
fn delta_patch_renderer(patch: &str) -> color_eyre::Result<String> {
    ...
    delta.stdin.as_mut().ok_or_else(|| eyre!("Failed to get stdin handle"))?.write_all(patch.as_bytes())?;
...
fn diff_so_fancy_renderer(patch: &str) -> color_eyre::Result<String> {
    ...
    dsf.stdin.as_mut().ok_or_else(|| eyre!("Failed to get stdin handle"))?.write_all(patch.as_bytes())?;
